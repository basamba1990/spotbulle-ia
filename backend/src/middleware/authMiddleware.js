...
// CORRECTION: Middleware d'authentification optionnel (n'échoue pas si pas de token)
const optionalAuth = async (req, res, next) => {
  try {
    ...
    // Si pas de token, continuer sans utilisateur authentifié
    if (!token) {
      console.log('ℹ️ Pas de token fourni pour route optionnelle:', req.path);
      req.user = null;
      return next();
    }
    ...
    try {
      ...
      console.log('⚠️ Token invalide pour route optionnelle:', jwtError.message);
      req.user = null;
      return next();
    }
    ...
    // Vérifier que le token contient les informations nécessaires
    if (!decoded.userId) {
      console.log('⚠️ Token sans userId pour route optionnelle');
      req.user = null;
      return next();
    }
    ...
    try {
      ...
      console.error('⚠️ Erreur DB lors de la récupération utilisateur (optionnel):', dbError);
      req.user = null;
      return next();
    }
    
    if (!user) {
      console.log('⚠️ Utilisateur non trouvé pour ID (optionnel):', decoded.userId);
      req.user = null;
      return next();
    }
    ...
    if (user.statut === 'suspendu' || user.statut === 'inactif') {
      console.log('⚠️ Utilisateur suspendu/inactif (optionnel):', user.id);
      req.user = null;
      return next();
    }
    ...
    console.log(`✅ Authentification optionnelle réussie pour: ${user.email} (${req.path})`);
    next();
    
  } catch (error) {
    console.error('⚠️ Erreur dans le middleware d\'authentification optionnel:', error);
